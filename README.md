...
**This physical-screen approach was eventually abandoned** as the way to track Claude usage. Even with the dimming/redraw-on-change refresh policy described below, a second physical display was more overhead than it was worth for something you glance at constantly while already looking at the terminal — so usage tracking moved to a Claude Code terminal statusline instead.

**→ See [`STATUSLINE.md`](STATUSLINE.md)** for what it looks like, what it's made of, and the steps to set it up in your own local Claude Code install.
